...
load_dotenv()
API_KEY = os.getenv("WebOfScienceAPIKey")
if not API_KEY:
    logging.error("Web of Science API key not found in environment variables.")
    raise ValueError("Web of Science API key not found. Please set WebOfScienceAPIKey in .env file.")

class WebOfScienceAPIError(APIError):
    """Custom exception for Web of Science API related errors."""
    pass

def validate_wos_response(response: requests.Response) -> Dict[str, Any]:
    """
    Validate and parse Web of Science API response.
    ...
    try:
        ...
        # Check for error messages in Web of Science response
        if 'error' in data:
            error_msg = data['error'].get('message', 'Unknown error')
            raise ResponseError(f"Web of Science API Error: {error_msg}")
...
def validate_wos_paper_entry(entry: Dict[str, Any]) -> Dict[str, Any]:
    """
    Validate and clean a Web of Science paper entry from the API response.
    
    Args:
        entry: Raw paper entry from Web of Science API
    ...
    # Authors - Web of Science provides authors as a list
    ...
    year_str = entry.get("Published.BiblioYear")
    ...
    # Publication name
    pub_name = entry.get("Source", {}).get("SourceTitle")
    ...
    # URL
    url = entry.get("Other", {}).get("Identifier.Eid")
    if url and isinstance(url, str):
        paper["url"] = f"https://www.webofscience.com/wos/woscc/full-record/{url}"
    else:
        paper["url"] = None
    
    # Citation count
    cited_count = entry.get("CitedReferenceCount")
    ...
    # Additional Web of Science-specific fields
    paper["wos_uid"] = entry.get("UID")
    paper["wos_doi"] = entry.get("DOI")
    paper["wos_abstract"] = entry.get("Abstract", {}).get("AbstractText")
    paper["wos_keywords"] = entry.get("Keywords", {}).get("Keyword")
    paper["wos_issn"] = entry.get("Source", {}).get("ISSN")
    paper["wos_volume"] = entry.get("Source", {}).get("Volume")
    paper["wos_issue"] = entry.get("Source", {}).get("Issue")
    paper["wos_pages"] = entry.get("Source", {}).get("Pages")
    paper["wos_document_type"] = entry.get("DocumentType")
    paper["wos_language"] = entry.get("Language")
    
    return paper

def get_web_of_science(query: str, start_year: int, end_year: int, output_dir: str = 'output', 
                       timeout: int = 30, max_retries: int = 3, save_bibtex: bool = True) -> pd.DataFrame:
    """
    Fetch papers from Web of Science API for given query and year range.
    ...
    logging.info(f"Starting Web of Science search for years: {start_year}-{end_year}, query: {query}")
    
    # Prepare output folder
    try:
        output_folder = os.path.join(os.getcwd(), output_dir, "web_of_science")
    ...
    full_query = f'TS=("{query}") AND PY=({start_year}-{end_year})'
    logging.info(f"Processing query: {full_query}")
    
    start_index = 1  # Web of Science uses 1-based indexing
    batch = 1
    ...
    while True:
        ...
        url = "https://ws.clarivate.com/api/wos/v1/search"
        headers = {
        ...
        params = {
            "usrQuery": full_query,
            "databaseId": "WOS",
            "count": 50,  # Web of Science max per request
            "firstRecord": start_index,
            "lang": "en"
        }
        ...
        while retries >= 0:
            try:
                ...
                # Validate Web of Science specific response
                data = validate_wos_response(response)
                ...
                records = data.get('Records', {}).get('REC', [])
                if not records:
                ...
                for record in records:
                    try:
                        paper = validate_wos_paper_entry(record)
                        if paper:  # Only add if validation passed
                            all_papers.append(paper)
                            valid_papers += 1
                    except Exception as e:
                        logging.warning(f"Failed to process record entry: {e}")
                        continue
                
                total_results = int(data.get('Records', {}).get('@total', 0))
                start_index += len(records)
                logging.info(f"Fetched {len(records)} records, {valid_papers} valid. Total so far: {len(all_papers)}. Next start: {start_index}/{total_results}")
                ...
                retries -= 1
                if retries < 0:
                    raise WebOfScienceAPIError(f"Rate limit exceeded: {e}")
                ...
                if consecutive_errors >= max_consecutive_errors:
                    raise WebOfScienceAPIError(f"Too many consecutive errors ({consecutive_errors})")
                time.sleep(5 * (max_retries - retries))  # Exponential backoff
                if retries < 0:
                    raise WebOfScienceAPIError(f"API response error: {e}")
                    
            except Exception as e:
                logging.error(f"Unexpected error: {e}")
                retries -= 1
                if retries < 0:
                    raise WebOfScienceAPIError(f"Unexpected error: {e}")
    ...
    try:
        # Save to CSV
        csv_filename = f'wos_papers_{start_year}_{end_year}.csv'
        csv_path = os.path.join(output_folder, csv_filename)
        final_df = save_results_to_csv(all_papers, csv_path)
        
        # Save to BibTeX if requested
        if save_bibtex:
            bibtex_filename = f'wos_papers_{start_year}_{end_year}.bib'
        ...
        raise WebOfScienceAPIError(f"Failed to process results: {e}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        get_web_of_science(example_query, example_start_year, example_end_year, save_bibtex=True)
    except WebOfScienceAPIError as e:
        logging.error(f"Web of Science API error: {e}")
        sys.exit(1)
